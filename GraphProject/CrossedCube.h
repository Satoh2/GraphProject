...
private:
	...
	//  * (u1, u0) ~ (v1, v0)
	bool DPPR(int u0, int u1, int v0, int v1, int sum);

	// Distance-preserving pair related
	//  * (node1_{index+1}, node1_{index}) ~ (node2_{index+1}, node2_{index})
	//  * ƒ°_{0 to index + 1}ƒÏ(node1, node2, index)
	bool DPPR(BinaryNode node1, BinaryNode node2, int index, int sum);
...
public:
	...
	void CalcDistanceClassification(BinaryNode node1, BinaryNode node2, int* distanceArray);
};